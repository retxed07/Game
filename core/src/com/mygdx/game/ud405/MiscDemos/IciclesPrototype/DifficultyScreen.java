package com.mygdx.game.ud405.MiscDemos.IciclesPrototype;
...
public class DifficultyScreen extends InputAdapter implements Screen {
    ...
    public DifficultyScreen(SplashScreen game) {
        this.game = game;
    }
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (worldTouch.dst(Constants.EASY_CENTER) < Constants.DIFFICULTY_BUBBLE_RADIUS) {
            game.setScreen(new IciclesScreen(game, Constants.Difficulty.EASY));
        }

        if (worldTouch.dst(Constants.MEDIUM_CENTER) < Constants.DIFFICULTY_BUBBLE_RADIUS) {
            game.setScreen(new IciclesScreen(game, Constants.Difficulty.MEDIUM));
        }

        if (worldTouch.dst(Constants.HARD_CENTER) < Constants.DIFFICULTY_BUBBLE_RADIUS) {
            game.setScreen(new IciclesScreen(game, Constants.Difficulty.HARD));
        }
